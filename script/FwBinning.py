...
class RebinLikelihood:
    ...
    def mergeCriteria(self, i):
        ...
        if self.h_bkg.GetBinContent(i) == 0 or self.h_bkg.GetBinError(i) / self.h_bkg.GetBinContent(i) > self.maxStat or self.h_sig.GetBinContent(i) <= 0 or self.h_sig.GetBinError(i) / self.h_sig.GetBinContent(i) > self.maxSigStat: return True
        else: return False
    # Recursive rebin function
    def directionalRebin(self, direction = ""):
        if direction == "":
    ...
    def applyMapping(self, mapping, h):
        ...
        #return self.applyMappingTrimmed(mapping, h)
