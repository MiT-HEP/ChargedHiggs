...
#        if 'HplusTB_HplusToTB' in self.nameUp or 'HplusTB_HplusToTB' in self.nameDown:
#            self.smoothGaussianKernel()
#        else:
#            self.smoothFit()
        self.smoothFit()
#        self.plot() # plot it
    ...
    def smoothFit(self):
        ...
        if self.rUp.GetNbinsX() < 6: return
    ...
    def smoothGaussianKernel(self):
        ...
        if self.rUp.GetNbinsX() == 1: return
        
        print "[SystematicSmoother] linear adaptive Gaussian kernel smoothing with sigma = %d" % (self.sigma)
        ...
        for i in range(1, self.rUp.GetNbinsX()+1):
            ...
            for j in range(1, self.rUp.GetNbinsX()+1):
            
                weightUp = math.exp( -(i-j)**2/(2.*self.sigma))
                weightDown = math.exp( -(i-j)**2/(2.*self.sigma))
    ...
    def plot(self):
        ...
        self.rUp.SetLineColor(ROOT.kBlue)
        self.rUp.SetLineColorAlpha(ROOT.kBlue, 0.4)
        self.rUp.SetMarkerColor(ROOT.kBlue)
        self.rUp.SetMarkerStyle(7)
    
        self.rDown.SetLineWidth(0)
        self.rDown.SetLineColor(ROOT.kRed)
        self.rDown.SetLineColorAlpha(ROOT.kRed, 0.4)
        self.rDown.SetMarkerColor(ROOT.kRed)
        self.rDown.SetMarkerStyle(7)
        ...
        self.rUpSmoothed.SetLineColor(ROOT.kBlue)
        self.rUpSmoothed.Draw("SAME HIST")
        self.rDownSmoothed.SetLineWidth(2)
        self.rDownSmoothed.SetLineColor(ROOT.kRed)
        ...
        self.rUp.GetYaxis().SetTitle("ratio")
        if self.binMax > 100: self.rUp.GetXaxis().SetTitle("H_{T} [GeV]")
        else: self.rUp.GetXaxis().SetTitle("MVA discriminator")
    ...
    def mergeCriteria(self):
        ...
        pos = -1
        for b in reversed(range(1, self.h.GetNbinsX()+1)):
