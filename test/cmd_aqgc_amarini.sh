...
python script/plotAQGC.py --more $(ls Datacards/OCT7/AQGC/higgs*all.root | while read f ; do echo  "$f:$(echo ${f%%_all.root} | sed 's:.*_::g')" ; done | tr '\n' ',' | sed 's/,$//') -o plot_aqgc/all  --xrange -16,16 -b

python script/plotAQGC_panel.py -o plot_aqgc/all
rsync -avP plot_aqgc/ ~/www/cms-private/VBSHadronic/OCT7/plot_aqgc/
