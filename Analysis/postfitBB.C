...
//	catep.push_back( (TDirectoryFile*)f1->Get(Form("shapes_fit_b/%s" ,cat.Data())) );
	...
	double xend = -1.;
		...
		tot_sig = (TH1D*)cater[i]->Get("VVEWK");
		tot_sig2 = (TH1D*)cater[i]->Get("VVEWK");
		tot_sig3 = (TH1D*)cater[i]->Get("VVEWK");
		...
		tot_sigbin3 = (TH1D*)cateorg[i]->Clone("sig3");
			...
			tot_sigbin3->SetBinContent(k,tot_sig3->GetBinContent(k)/xwidth);
                ...
                tot_sigbin3->SetLineColor(kBlue+2);

		tot_sigbin->SetLineWidth(2);
                tot_sigbin->SetLineColor(kRed+1);

		tot_sigbin2->SetLineWidth(2);
                tot_sigbin2->SetLineColor(kViolet);
		...
		//tot_bkg
			...
			if(tot_bkg->GetBinContent(k)/cateorg[i]->GetBinWidth(k) > upperlim) upperlim = tot_bkg->GetBinContent(k)/cateorg[i]->GetBinWidth(k);
                        ...
                        if (NBins == 1){cout << "1 la" << endl;
                        	xwidth = 1.;
                                bkgall->SetPoint(k-1,0.5,tot_bkg->GetBinContent(k)/xwidth);
                                bkgall->SetPointError(k,0.5,tot_bkg->GetBinError(k)/xwidth);
                                HhRatio->SetPoint(k-1,0.5,1.0);
                                HhRatio->SetPointError(k-1,0.5,tot_bkg->GetBinError(k)/tot_bkg->GetBinContent(k));
                        //}else if(k == 1){
                        //        xwidth-=500;
		...
		bkgall->SetFillStyle(3004);
			...
			if(ht[j] != NULL) cout << bkgname[j] << ": " << ht[j]->Integral() << endl;
		...
		for(int k=0; k<NBins; k++){
                	...
                	double xbincenter = cateorg[i]->GetXaxis()->GetBinCenter(k+1);
                        ...
                        double xwidth = cateorg[i]->GetBinWidth(k+1);
                        if (NBins == 1){xwidth = 0.5;xbincenter=0.5; xbinerr=0.5;}
	...
	cout << "zenmel0" << endl;
        ...
        cateyields->SetMinimum(lowerlim/100);
        //cateyields->GetYaxis()->SetLimits(1.,16000.0);
        cateyields->GetYaxis()->SetTitle("Events/DNN");
...
*/
        ...
        TLegend *legend0 = new TLegend(0.4,0.7,0.64,0.89);
        legend0->SetFillStyle(0);
        legend0->SetBorderSize(0);
        legend0->AddEntry(bkgall,"Bkg. unc.","f");

	legend0->AddEntry(tot_sigbin,"VVEWK","l");
	...
	//legend0->AddEntry(tot_sigbin2,"QCD Z#nu#nuWqq","l");
        //legend0->AddEntry(tot_sigbin3,"EWK Z#nu#nuZqq","l");
	...
	cout << "zenmel1" << endl;
        ...
        TLegend *legend1 = new TLegend(0.65,0.6,0.95,0.88);
