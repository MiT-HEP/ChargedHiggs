...
float SimpleJetCorrectionUncertainty::uncertainty(const std::vector<float>& fX, float fY, bool fDirection) const 
{
  ...
  if (bin<0) {
    Logger::getInstance().Log("SimpleJetCorrectionUncertainty",__FUNCTION__,"ERROR","bin variables out of range");
    result = -999.0;
  } else 
...
float SimpleJetCorrectionUncertainty::uncertaintyBin(unsigned fBin, float fY, bool fDirection) const 
{
  if (fBin >= mParameters->size()) { 
    Logger::getInstance().Log("SimpleJetCorrectionUncertainty",__FUNCTION__,"ERROR",Form("wrong bin: %u: only %u are available",fBin,mParameters->size()));
    return -999.0;
  }
  ...
  if (fX[0] == fX[1])
    {
      if (fY[0] == fY[1])
        ...
        Logger::getInstance().Log("SimpleJetCorrectionUncertainty",__FUNCTION__,"ERROR","interpolation error");
	return -999.0;
      }
    } 
  else   
