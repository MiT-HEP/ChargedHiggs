  ...
  smearingType_(ECALELF)
{
  ...
  if(!correctionFileName.empty()) { 
    ...
    if(scales_.empty()) {
      ...
      Logger::getInstance().Log("EnergyScaleCorrection",__FUNCTION__,"ERROR","scale correction map empty"); 
  ...
  if(!correctionFileName.empty()) { 
    ...
    if(smearings_.empty()) {
      ...
      Logger::getInstance().Log("EnergyScaleCorrection",__FUNCTION__,"ERROR","smearing correction map empty"); 
      throw abortException();
    }
  }
				    ...
				    unsigned int gainSeed) const
{
  ...
  if(nrFound==0){
    //edm::LogInfo("EnergyScaleCorrection") << "Scale category not found: " << category << " Returning uncorrected value.";
      Logger::getInstance().Log("EnergyScaleCorrection",__FUNCTION__,"INFO",Form("scale error category ??? has  %d entries",nrFound)); 
    return nullptr;
  }else if(nrFound>1){
    ...
    for(auto it = result.first;it!=result.second;++it){
      ...
      Logger::getInstance().Log("EnergyScaleCorrection",__FUNCTION__,"ERROR",Form("scale error category ??? has  %d entries",nrFound)); 
      throw abortException();
  }  
  ...
  if(!result.first->first.inCategory(runnr,et,eta,r9,gainSeed)){
      ...
      Logger::getInstance().Log("EnergyScaleCorrection",__FUNCTION__,"ERROR","error found scale category that does not contain run et eta r9..."); 
      throw abortException();
  }
				    ...
				    unsigned int gainSeed) const
{
  ...
  if(nrFound==0){
    ...
    Logger::getInstance().Log("EnergyScaleCorrection",__FUNCTION__,"INFO",Form("Smear category not found. Returting uncorrected value")); 
    return nullptr;
  }else if(nrFound>1){
    std::ostringstream foundCats;
    for(auto it = result.first;it!=result.second;++it){
    ...
    Logger::getInstance().Log("EnergyScaleCorrection",__FUNCTION__,"ERROR",Form("smear error category ??? has  %d entries",nrFound)); 
    throw abortException();
  }
  //validate the result, just to be sure
  if(!result.first->first.inCategory(runnr,et,eta,r9,gainSeed)){
    ...
    Logger::getInstance().Log("EnergyScaleCorrection",__FUNCTION__,"ERROR",Form(" error found smear category  that does not contain run et ...")); 
    throw abortException();
  }
  return &result.first->second;
}
				     ...
				     double energyScaleErrSyst, double energyScaleErrGain)
{
  ...
  if(result.first!=result.second){
    ...
    Logger::getInstance().Log("EnergyScaleCorrection",__FUNCTION__,"ERROR","Category already defined"); 
    throw abortException();
  }
					...
					double eMean, double errEMean)
{
  ...
  if(res.first!=res.second) {
    ...
    Logger::getInstance().Log("EnergyScaleCorrection",__FUNCTION__,"ERROR","Smearing Category already defined"); 
    throw abortException();
  }
...
void EnergyScaleCorrection::readScalesFromFile(const std::string& filename)
{
  ...
  std::string path= ("aux/ScalesSmearings/" + filename).c_str();
  std::ifstream file(path.c_str());
  
  if(!file.good()) {
    //throw cms::Exception("EnergyScaleCorrection") << "file " << filename << " not readable.";
    Logger::getInstance().Log("EnergyScaleCorrection",__FUNCTION__,"ERROR",Form("File %s not readable",path.c_str())); 
    throw abortException();
  }
...
void EnergyScaleCorrection::readSmearingsFromFile(const std::string& filename)
{
  ...
  while(file.peek() != EOF && file.good()) {
    ...
    Logger::getInstance().Log("EnergyScaleCorrection",__FUNCTION__,"ERROR","unknown smearing type"); 
    throw abortException();
